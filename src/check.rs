...
fn parse_cert_der(host: &str, port: u16, der: &[u8]) -> Result<CertInfo, TlsdaysError> {
    ...
    let not_before = cert
        ...
        .not_before
        .to_datetime()
        .map_err(|e| TlsdaysError::CertParse {
            host: host.to_string(),
            port,
            message: format!("invalid not_before: {e}"),
        })?;
    let not_after = cert
        .validity()
        .not_after
        ...
        .map_err(|e| TlsdaysError::CertParse {
            host: host.to_string(),
            port,
            message: format!("invalid not_after: {e}"),
        })?;

    let not_before: DateTime<Utc> = not_before.into();
    let not_after: DateTime<Utc> = not_after.into();
    let now = Utc::now();
...
pub async fn check_host(host: &str, port: u16, connect_timeout: Duration) -> Result<CertInfo, TlsdaysError> {
    let addrs: Vec<_> = {
    ...
    let server_name = ServerName::try_from(host.to_string()).map_err(|_| TlsdaysError::Argument(format!(
        "invalid DNS name for TLS SNI: {host}"
    )))?;
    ...
    for addr in addrs {
        ...
        let certs = session.peer_certificates().ok_or_else(|| TlsdaysError::Tls {
            host: host.to_string(),
            port,
    ...
    Err(TlsdaysError::Tcp {
        ...
        source: last_tcp_err.unwrap_or_else(|| {
            std::io::Error::new(std::io::ErrorKind::ConnectionRefused, "all resolved addresses failed")
        }),
    })
}
